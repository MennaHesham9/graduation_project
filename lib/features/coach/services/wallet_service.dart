...
class WalletService {
  ...
  Future<WalletSummary> fetchSummary(String coachId) async {
    ...
    final snap = await _db
        ...
        .where('status', whereIn: ['confirmed', 'completed', 'rescheduled'])
    ...
    for (final doc in snap.docs) {
      ...
      // The date shown in the transaction history
      final scheduledAt = (data['scheduledAtUtc'] as Timestamp?)?.toDate() ?? now;

      // The date the money was actually paid (used for financial month tracking)
      final bookedAt = (data['createdAt'] as Timestamp?)?.toDate() ?? scheduledAt;
      ...
      if (bookedAt.isAfter(monthStart)) {
  ...
  Stream<WalletSummary> summaryStream(String coachId) {
    return _db
        .collection('sessions')
        .where('coachId', isEqualTo: coachId)
        // 1. ADD 'rescheduled' to the allowed statuses
        .where('status', whereIn: ['confirmed', 'completed', 'rescheduled'])
        // 2. IMPORTANT: We still order by scheduledAtUtc for the UI list,
        // but we will use createdAt for the money logic.
        .orderBy('scheduledAtUtc', descending: true)
        .snapshots()
        .map((snap) {
          final now = DateTime.now().toUtc();
          ...
          for (final doc in snap.docs) {
            ...
            final bookedAt =
                (data['createdAt'] as Timestamp?)?.toDate() ?? scheduledAt;
            final clientName =
