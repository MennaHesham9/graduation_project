...
class _CreateNewGoalScreenState extends State<CreateNewGoalScreen> {
  ...
  // UPDATED: Return to Goal Dashboard
  void _save() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Goal Saved Successfully!')),
    );
    // Returns to the previous screen (GoalsDashboardScreen)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column( // Removed Center and Container constraints
          children: [
            // Top Header
            Padding(
                  ...
                  children: [
                    ...
                    _SectionCard(
                      ...
                      trailing: InkWell(
                        ...
                        child: const Text(
                          '+ Add Step',
                          style: TextStyle(color: Color(0xFF1B9AAA), fontWeight: FontWeight.bold),
                        ),
                      ),
                      child: Column(
                        children: [
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Save Goal', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF1B9AAA),
                    foregroundColor: Colors.white,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
...
class _InputField extends StatelessWidget {
  ...
  const _InputField({required this.controller, required this.hintText, this.maxLines = 1, this.prefixIcon});
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                Text(value.isEmpty ? 'Select' : value, style: const TextStyle(fontWeight: FontWeight.bold)),
              ],
            ),
          ),
        ),
...
class _CategoryGrid extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final categories = ['Personal Growth', 'Career', 'Mental Health', 'Financial', 'Fitness'];
    return Wrap(
      spacing: 8,
      runSpacing: 8,
...
class _StepRow extends StatelessWidget {
  ...
  final bool done;
  final VoidCallback onTap;
  const _StepRow({required this.text, required this.done, required this.onTap});

  @override
  Widget build(BuildContext context) {
    return ListTile(
      contentPadding: EdgeInsets.zero,
      leading: Icon(done ? Icons.check_circle : Icons.circle_outlined, color: const Color(0xFF1B9AAA)),
      title: Text(text),
      onTap: onTap,
    );
  }
...
class _StepItem {
  final String text;
  final bool done;
  const _StepItem({required this.text, required this.done});
  _StepItem copyWith({bool? done}) => _StepItem(text: text, done: done ?? this.done);
}