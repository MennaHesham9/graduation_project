...
//   _load() was called directly from initState(). Even though the provider's
//   load() guards notifyListeners(), the call chain can trigger a rebuild of
//   _InheritedProviderScope while the parent widget tree (ClientHomeScreen)
//   is still in its first build pass. Wrapping _load() in a
//   WidgetsBinding.instance.addPostFrameCallback ensures it runs only after
//   the initial frame is fully committed.
...
class _MentalWellnessDashboardScreenState
  ...
  void initState() {
    super.initState();
    // FIX: Use addPostFrameCallback so _load() is called after the first build
    // frame completes. Calling it directly from initState() caused
    // "setState() or markNeedsBuild() called during build" because the
    // DashboardProvider notified its listeners while ClientHomeScreen was
    // still constructing its widget tree.
                        ...
                        Positioned(
                          ...
                          child: GestureDetector(
                            ...
                            child: NextSessionCard(
                              ...
                              sessionId: data.nextSession.sessionId,
                              scheduledAt: data.nextSession.scheduledAt,
                              isVideoSession: data.nextSession.sessionType == 'video',
