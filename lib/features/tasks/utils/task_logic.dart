...
String todayCompletionKey() => _dateKey(DateTime.now());
...
/// Always shows progress out of the FULL task range (start → end),
/// so a 30-day daily task shows "1/30 (3%)" on day 1 after one completion.
...
/// How many total occurrences does this task have over its FULL date range?
///
/// Key change: uses [endDate] (or a fallback of today) as the upper bound —
/// NOT capped at today. This means on day 1 of a 30-day daily task the
/// denominator is already 30, giving the coach full visibility into the
/// client's overall progress trajectory from the start.
///
/// For tasks with no end date, falls back to today so the denominator
/// stays meaningful and doesn't grow to infinity.
int expectedOccurrences(TaskModel task) {
  ...
  // Use the real end date if set, otherwise fall back to today
  ...
  switch (task.repetition) {
    ...
    case RepetitionType.daily:
    // Full day count from start to end (inclusive)
